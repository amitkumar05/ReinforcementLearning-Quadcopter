...
class Hover(BaseTask):
    ...
    def reset(self):
        ...
        return Pose(
                position=Point(10.0, 10.0, np.random.normal(20, 0.1)),  # drop off from a slight random height
                orientation=Quaternion(0.0, 0.0, 0.0, 0.0),
            ), Twist(
                linear=Vector3(0.0, 0.0, 5.0),
    ...
    def update(self, timestamp, pose, angular_velocity, linear_acceleration):
        ...
        if abs(pose.position.z-self.target_z) >= 5 :  # agent has gone deviated from the target height
